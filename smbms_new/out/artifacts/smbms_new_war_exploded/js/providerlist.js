...
function deleteProvider(obj){
	$.ajax({
		type:"DELETE",
		url:path+"/sys/deleteprovider/"+obj.attr("proid"),
		dataType:"json",
...
$(function(){
	$(".viewProvider").on("click",function(){
		...
		var obj = $(this);
		window.location.href=path+"/sys/provider/"+ obj.attr("proid");
	});
	
	$(".modifyProvider").on("click",function(){
		var obj = $(this);
		window.location.href=path+"/sys/providermodify/"+ obj.attr("proid");
	});
